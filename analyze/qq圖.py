...
while True:
    ...
    if display==1:
        ...
        for num in displayprint:
            print(num)
    displayprint = np.array(displayprint)
    y=displayprint.sort()
    x_values = range(len(displayprint))
    plt.plot(x_values, displayprint, label="Data")
    plt.plot([0, len(displayprint) - 1], [0, max(displayprint)], linestyle="--", color="red", label="45-degree Line")
    plt.xlabel("Index")
    plt.ylabel("Values")
    plt.title("Data Plot with 45-degree Reference Line")
    plt.legend()
    plt.show()
